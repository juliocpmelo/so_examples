#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/types.h> 
#include <sys/socket.h>
#include <netinet/in.h>
#include <errno.h>
#include <fcntl.h>


void error(const char *msg)
{
	perror(msg);
	exit(1);
}

int main(int argc, char *argv[])
{
	int sockfd, newsockfd, portno;
	socklen_t clilen;
	char buffer[256];
	struct sockaddr_in serv_addr, cli_addr;
	int n;
	if (argc < 2) {
		fprintf(stderr,"ERROR, no port provided\n");
		exit(1);
	}
	/*cria um socket de internet do tipo stream (tcp)*/
	sockfd = socket(AF_INET, SOCK_STREAM, 0);
	if (sockfd < 0) 
		error("ERROR opening socket");

	/*seta o socket para nao bloqueante*/
	fcntl(sockfd, F_SETFL, O_NONBLOCK);

	bzero((char *) &serv_addr, sizeof(serv_addr));
	portno = atoi(argv[1]);
	/**
	 * inicializa a estrutura serv_addr no servidor:
	 * sin_family = Internet
	 * s_addr = INADDR_ANY, qualquer ip pode se conectar
	 * sin_port = porta
	 **/
	serv_addr.sin_family = AF_INET;
	serv_addr.sin_addr.s_addr = INADDR_ANY;
	serv_addr.sin_port = htons(portno);
	/*bind cria um servico na máquina corrente na porta especificada*/
	if (bind(sockfd, (struct sockaddr *) &serv_addr,
				sizeof(serv_addr)) < 0) 
		error("ERROR on binding");
	
	/*enumera o número máximo de conexoes que esse servidor poderá receber
	 * ao mesmo tempo*/
	listen(sockfd,5);
	clilen = sizeof(cli_addr);
	
	/*bloqueia até que uma nova conexao seja recebida*/

	/*nesse caso, accept nao bloqueia, entao, a unica forma que
	 * temos para verificar a nao ocorrencia de erros é
	 * checando o errno*/

	puts("Waiting for connections...");	
	while(1){
		newsockfd = accept(sockfd, 
				(struct sockaddr *) &cli_addr, 
				&clilen);
		if (newsockfd < 0){
			if(errno == EAGAIN  || errno == EWOULDBLOCK){
				puts("Waiting for connections...");	
				usleep(500000);
			}
			else{
				error("ERROR on accept");
			}
		}
		else{
			break;
		}
	}

	bzero(buffer,256);
	


	/*lê mensagens desse socket*/
	puts("Waiting for messages...");	
	while(1){
		n = read(newsockfd,buffer,255);
		if (n < 0){ 
			if(errno == EAGAIN  || errno == EWOULDBLOCK){
				puts("Waiting for messages...");	
				usleep(500000);
			}
			else{
				error("ERROR reading from socket");
			}
		}
		else{
			break;
		}
	}
	
	printf("Here is the message: %s\n",buffer);

	/*escreve mensagens nesse socket*/
	n = read(newsockfd,buffer,255);
	n = write(newsockfd,"I got your message",18);
	if (n < 0) error("ERROR writing to socket");
	close(newsockfd);
	close(sockfd);
	return 0; 
}
